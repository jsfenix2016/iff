...
class PreviewActivitiesByDate extends StatefulWidget {
  ...
  @override
  State<PreviewActivitiesByDate> createState() => _PreviewActivitiesByDateState();
}

class _PreviewActivitiesByDateState extends State<PreviewActivitiesByDate> with WidgetsBindingObserver {

  final AddActivityController controller = Get.put(AddActivityController());

  final List<String> tempNoSelectListDay = <String>["L", "M", "X", "J", "V", "S", "D",];
  ...
  final Map<int, List<ActivityDay>> _activitiesByDay = <int, List<ActivityDay>>{};
  ...
  void filterActivitiesByRangeDate() async {
    ...
    for (var index=0;index<=daysBetweenToAndFrom;index++) {
      ...
      for (var activity in _activities) {
        ...
        var endDate = Jiffy(activity.dayFinish.toLowerCase(), 'EEEE, d MMMM yyyy');
        ...
        if (endDate.isBefore(_from!) || startDate.isAfter(_to!)) {}
          ...
          if (activity.specificDaysRemoved != null && activity.specificDaysRemoved!.isNotEmpty) {
            var daysRemoved = activity.specificDaysRemoved!.split(';');
            for (var dayRemoved in daysRemoved) {
              var dayRemovedDate = Jiffy(dayRemoved.toLowerCase(), 'EEEE, d MMMM yyyy').dateTime;
              if (dayRemovedDate.year == currentDate.year && dayRemovedDate.month == currentDate.month
          ...
          if (!isDayRemoved) {
            ...
            switch (activity.repeatType) {
              case onceTime:
                var onceDate = Jiffy(activity.day.toLowerCase(), getDefaultPattern()).dateTime;
                if (onceDate.year == currentDate.year && onceDate.month == currentDate.month
                    && onceDate.day == currentDate.day && isFilteredDayValid(onceDate)) {
                  activitiesByDay.add(activity);
                }
                break;
              case diary:
                var diaryStartDate = Jiffy(activity.day.toLowerCase(), getDefaultPattern()).dateTime;
                var diaryEndDate = Jiffy(activity.dayFinish.toLowerCase(), getDefaultPattern()).dateTime;

                if (!diaryStartDate.isAfter(currentDate) && !diaryEndDate.isBefore(currentDate)
              ...
              case weekly:
                if (activity.days != null && activity.days!.isNotEmpty) {
                  ...
                  for (var day in daysRepeated) {
                    var weeklyStartDate = Jiffy(activity.day.toLowerCase(), getDefaultPattern()).dateTime;
                    var weeklyEndDate = Jiffy(activity.dayFinish.toLowerCase(), getDefaultPattern()).dateTime;

                    if (!weeklyStartDate.isAfter(currentDate) && !weeklyEndDate.isBefore(currentDate)
                        && isFilteredDayValid(currentDate)) {
                      for (var index=0;index<tempNoSelectListDay.length;index++) {
                        if (currentDate.weekday == (index+1) && day == tempNoSelectListDay[index]) {
                          activitiesByDay.add(activity);
                          break;
                        }
                      }
                ...
                break;
              case monthly:
                var monthlyStartDate = Jiffy(activity.day.toLowerCase(), getDefaultPattern()).dateTime;
                var monthlyEndDate = Jiffy(activity.dayFinish.toLowerCase(), getDefaultPattern()).dateTime;

                if (!monthlyStartDate.isAfter(currentDate) && !monthlyEndDate.isBefore(currentDate)
                    && isFilteredDayValid(currentDate)) {
                ...
                break;
              case yearly:
                var yearlyStartDate = Jiffy(activity.day.toLowerCase(), getDefaultPattern()).dateTime;
                var yearlyEndDate = Jiffy(activity.dayFinish.toLowerCase(), getDefaultPattern()).dateTime;

                if (!yearlyStartDate.isAfter(currentDate) && !yearlyEndDate.isBefore(currentDate)
                    && isFilteredDayValid(currentDate)) {
                  if (yearlyStartDate.month == currentDate.month && yearlyStartDate.day == currentDate.day) {
                    activitiesByDay.add(activity);
                  }
                }
                break;
      ...
      if (activitiesByDay.isNotEmpty) {
        ...
        Map<int, List<ActivityDay>> values = { _days.length - 1 : activitiesByDay };
  ...
  bool isFilteredDayValid(DateTime dateTime) {
    ...
    for (var index=0;index<_selectedDays.length;index++) {
      if (_selectedDays.contains(true) && !_selectedDays[index] && dateTime.weekday == (index+1)) {
        isDayValid = false;
        break;
      }
    }
  ...
  Future<bool> isDeactivated(ActivityDay activityDay, DateTime dateTime) async {
    ...
    if (activityDay.specificDaysDeactivated != null && activityDay.specificDaysDeactivated!.isNotEmpty) {
      var specifyDaysDeactivated = activityDay.specificDaysDeactivated!.split(';');

      for (var specifyDayDeactivated in specifyDaysDeactivated) {
        var specifyDayDateTime = Jiffy(specifyDayDeactivated.toLowerCase(), getDefaultPattern()).dateTime;

        if (specifyDayDateTime.year == dateTime.year && specifyDayDateTime.month == dateTime.month
  ...
  void addIsDeactivated(ActivityDay activityDay, DateTime dateTime) {
    ...
    if (activityDay.specificDaysDeactivated != null && activityDay.specificDaysDeactivated!.isNotEmpty) {
      activityDay.specificDaysDeactivated = "${activityDay.specificDaysDeactivated};$deactivatedDateTime";
    } else {
  ...
  void removeIsDeactivated(ActivityDay activityDay, DateTime dateTime) {
    ...
    for (var specifiedDayDeactivated in array) {
      if (deactivatedDateTime != specifiedDayDeactivated) {
        if (activityDay.specificDaysDeactivated != null && activityDay.specificDaysDeactivated!.isNotEmpty) {
          activityDay.specificDaysDeactivated = "${activityDay.specificDaysDeactivated};$specifiedDayDeactivated";
        } else {
  ...
  void addIsRemoved(ActivityDay activityDay, DateTime dateTime) {
    ...
    if (activityDay.specificDaysRemoved != null && activityDay.specificDaysRemoved!.isNotEmpty) {
      activityDay.specificDaysRemoved = "${activityDay.specificDaysRemoved};$removedDateTime";
    } else {
  ...
  void updateActivitiesName() async {
    ...
    if (_activitiesNameText.isNotEmpty) {
      _activitiesNameText = _activitiesNameText.substring(0, _activitiesNameText.length - 2);
    }
            ...
            Column(
              children: [
                Padding(
                  ...
                  child: Text(
                      "¿En qué actividades no utilizas tu smartphone?",
                              ...
                              children: [
                                Padding(
                                    ...
                                    child: Align(
                                      ...
                                      child: Text(
                                        Jiffy(_days[index]).format(getShortPattern()).capitalizeFirst!,
                                        textAlign: TextAlign.left,
                                        style: GoogleFonts.barlow(
                                          fontSize: 16.0,
                                          wordSpacing: 1,
                                              ...
                                              children: [
                                                ...
                                                Positioned(
                                                  ...
                                                  child: IconButton(
                                                    ...
                                                    onPressed: () {
                                                      addIsRemoved(
                                                          _activitiesByDay[index]![indexActivity],
                                                          _days[index]
                                                      );
                                                      controller.updateActivity(_activitiesByDay[index]![indexActivity]);
                                                      _activitiesByDay[index]!.removeAt(indexActivity);
                                                      setState(() {});
                                                      filterActivitiesByRangeDate();
                                                      updateActivitiesName();
                                                    },
