...
class AddActivityController extends GetxController {
  ...
  Future<ActivityDay> convertActivityDayBDToActivityDay(ActivityDayBD activityBD) async {
  ...
  Future<ActivityDayApi> convertToApi(ActivityDay activity) async {
    ...
    activityDayApi.disabledDates = await _convertSpecificDaysToList(activity.specificDaysDeactivated);
    activityDayApi.removedDates = await _convertSpecificDaysToList(activity.specificDaysRemoved);
  ...
  Future<List<DateTime>> _convertSpecificDaysToList(String? specificDays) async {
    await Jiffy.locale('es');
    ...
    if (specificDays != null && specificDays.isNotEmpty) {
      ...
      for (var specificDay in specifyDays) {
        var specificDateTime = Jiffy(specificDay.toLowerCase(), getDefaultPattern()).dateTime;
  ...
  Future<List<ActivityDay>> convertToApk(List<ActivityDayApiResponse> activitiesApi) async {
    await Jiffy.locale("es");
    List<ActivityDay> activities = [];
    ...
    for (var activityApi in activitiesApi) {
      ...
      activityDay.specificDaysDeactivated = await _convertListToSpecificDays(activityApi.disabledDates);
      activityDay.specificDaysRemoved = await _convertListToSpecificDays(activityApi.removedDates);
  ...
  Future<String?> _convertListToSpecificDays(List<DateTime>? specificDays) async {
    await Jiffy.locale('es');
    ...
    if (specificDays != null && specificDays.isNotEmpty) {
      ...
      if (strSpecificDays.isNotEmpty) strSpecificDays = strSpecificDays.substring(0, strSpecificDays.length -1);
