...
class Habits {
  ...
  final AddActivityController addActivityController = Get.put(AddActivityController());
...
//
      ...
      if (isValidDate(activity)) {
        if (isOutOfRestTime(activity) && await isOutOfActivityDay(activity)) {
          if (previousLogActivity != null &&
              isAfterWakeUp(activity) && isPreviousBeforeSleep(activity, previousLogActivity!)) {
            addActivityTimeBetweenDays(activity);
          } else if (previousLogActivity != null &&
              isBeforeActivity(previousLogActivity!) && isAfterActivity(activity)) {
  ...
  DateTime validDate() {
    ...
    DateTime valid = DateTime(now.year, now.month, now.day - maxValidDays, now.hour, now.minute);
    return valid;
  }
  ...
  bool isOutOfRestTime(LogActivity logActivity) {
    ...
    return logActivity.dateTime.isAfter(wakeUp) && logActivity.dateTime.isBefore(sleep);
  }
  ...
  Future<bool> isOutOfActivityDay(LogActivity logActivity) async {
    ...
    for (var activityDay in activityDays) {
      print("activityDay: " + activityDay.activity + "   " + activityDay.repeatType
      + activityDay.isDeactivate.toString());
      if (!activityDay.isDeactivate) {
        var startDate = Jiffy(activityDay.day.toLowerCase(), 'EEEE, d MMMM yyyy').dateTime;
        var endDate = Jiffy(activityDay.dayFinish.toLowerCase(), 'EEEE, d MMMM yyyy').dateTime;
        ...
        if (!logActivity.dateTime.isBefore(startDate) && !logActivity.dateTime.isAfter(endDate)) {
          print("Activity in: " + logActivity.dateTime.toString());
          if (!isDayRemoved(activityDay, logActivity.dateTime) && !isDayDeactivated(activityDay, logActivity.dateTime)) {
            if (isInSameDay(activityDay, logActivity.dateTime)) {
              var start = parseTime(activityDay.timeStart, logActivity.dateTime);
              ...
              print("isInActivityDayTime dateTime: " + logActivity.dateTime.toString());
              if (isInActivityDayTime(start, end, logActivity.dateTime)) {
                print("isInActivityDayTime dateTime: ok");
                currentActivityDay = activityDay;
                currentActivityDay!.day = Jiffy(start).format(getDefaultPattern());
                currentActivityDay!.dayFinish = Jiffy(end).format(getDefaultPattern());
  ...
  bool isBeforeActivity(LogActivity logActivity) {
    if (currentActivityDay != null) {
      var startDate = Jiffy(currentActivityDay!.day.toLowerCase(), 'EEEE, d MMMM yyyy').dateTime;
  ...
  bool isAfterActivity(LogActivity logActivity) {
    if (currentActivityDay != null) {
      var endDate = Jiffy(currentActivityDay!.dayFinish.toLowerCase(), 'EEEE, d MMMM yyyy').dateTime;
  ...
  bool isInSameDay(ActivityDay activityDay, DateTime dateTime) {
    switch (activityDay.repeatType) {
      case onceTime:
        var onceDate = Jiffy(activityDay.day.toLowerCase(), getDefaultPattern()).dateTime;
      ...
      case weekly:
        if (activityDay.days != null && activityDay.days!.isNotEmpty) {
          ...
          for (var day in daysRepeated) {
            for (var index = 0; index < Constant.tempListShortDay.length; index++) {
        ...
        return false;
      case monthly:
        var monthlyStartDate = Jiffy(activityDay.day.toLowerCase(), getDefaultPattern()).dateTime;
        ...
        return false;
      case yearly:
        var yearlyStartDate = Jiffy(activityDay.day.toLowerCase(), getDefaultPattern()).dateTime;

        if (yearlyStartDate.month == dateTime.month && yearlyStartDate.day == dateTime.day) {
          return true;//isInActivityDayTime(activityDay, dateTime, dateTime);
        }
        return false;
    }
...
//
  ...
  bool isPreviousBeforeSleep(LogActivity currentLogActivity, LogActivity logActivity) {
    RestDay restDay = getRestDay(logActivity.dateTime.weekday);
    DateTime sleep = parseTime(restDay.timeSleep, logActivity.dateTime);
    if (sleep.hour == 0 && sleep.minute == 0) {
      sleep = DateTime(sleep.year, sleep.month, sleep.day + 1);
  ...
  void addActivityTimeBetweenActivity(LogActivity activity) {
    int diffBefore = getTimeBetweenMovementAndStartActivity(previousLogActivity!);
  ...
  void addActivityTime(LogActivity activity) {
    if (movements > -1) {
      if (activity.dateTime.difference(previousLogActivity!.dateTime).inMinutes > minMinutes) {
        var minutes = activity.dateTime.difference(previousLogActivity!.dateTime).inMinutes;
        result += minutes;
        movements++;
